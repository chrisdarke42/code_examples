...
function getPDFFileNameFromURL(url) {
  var defaultFilename = arguments.length > 1 && arguments[1] !== undefined && arguments[1].length > 1 ? arguments[1] : 'document.pdf';
  ...
  // Try to extract a "file.pdf" portion of the URL.
  var reURI = /^(?:(?:[^:]+:)?\/\/[^\/]+)?([^?#]*)(\?[^#]*)?(#.*)?$/;
  var reFilename = /[^\/?#=]+\.pdf\b(?!.*\.pdf\b)/i;
  var splitURI = reURI.exec(url);
  var suggestedFilename1 = reFilename.exec(splitURI[1]);
  var suggestedFilename2 = reFilename.exec(splitURI[2]);
  var suggestedFilename3 = reFilename.exec(splitURI[3]);
  var suggestedFilename = suggestedFilename1 || suggestedFilename2 || suggestedFilename3;
  ...
  // If that failed, try to extract the last portion of the URL, which should be
  // a file ID. If that works, append ".pdf" to the string and call it a day.
  else if (splitURI[1] !== undefined) {
    var newRE = /^\/pdfs\/(.*)$/;
    var newSplit = newRE.exec(splitURI[1]);
    if (newSplit[1] !== undefined) {
      suggestedFilename = newSplit[1] + '.pdf';
    }
  }
...
function attachDOMEventsToEventBus(eventBus) {
  eventBus.on('webViewerLoaded', function () {
...
function configure(PDFJS) {
  ...
  PDFJS.cMapUrl = '/sites/all/libraries/pdf.js/web/cmaps/';
...
var PDFViewerApplication = {
  ...
  _initializeViewerComponents: function _initializeViewerComponents() {
    ...
    return new Promise(function (resolve, reject) {
      ...
      _this2.eventBus.dispatch('webViewerLoaded', { source: _this2 });
  ...
  get loadingBar() {
    this.eventBus.dispatch('renderLoadingBar');
  ...
  open: function open(file, args) {
    ...
    var updated = false;
    var fileaccess = true;
    var sidebaraccess = true;

    if (window.frameElement != null) {
      var pagecount = parseInt(window.frameElement.getAttribute('data-pagecount'), 10);
      if (pagecount) {
        // This is a PDF being viewed in the context of a Collection File, provisionally
        // limit access to download and printing.
        fileaccess = false;
        _this3.toolbar.setPagesCount(pagecount, false);
      }
    }
    if (window.frameElement.getAttribute('data-downloadable') == 1) {
      fileaccess = true;
    }

    var pagenum = null;
    if (args && (args.pagenum && typeof args.pagenum === 'number')) {
      pagenum = parseInt(args.pagenum);
      updated = true;
    }
    else {
      if (window.frameElement != null) {
        pagenum = parseInt(window.frameElement.getAttribute('data-page'), 10);
      }
    }

    if (pagenum) {
      // This is a single page output, grant access to Download and Print
      fileaccess = true;
      sidebaraccess = false;
      _this3.toolbar.setExternal = true;
      _this3.toolbar.pageNumber = pagenum;
    }

    if (updated) {
      _this3.toolbar._updateUIState(true);
    }

    if (!fileaccess || !sidebaraccess) {
      var appConfig = _this3.appConfig;
      if (!fileaccess) {
    ...
    return loadingTask.promise.then(function (pdfDocument) {
      _this3.eventBus.dispatch('webViewerLoaded', { source: _this3 });
  ...
  download: function download() {
    ...
    downloadManager.onerror = function (err) {
      _this4.error("PDF failed to download: ".concat(err));
    };
    ...
    if (percent > this.loadingBar.percent || isNaN(percent)) {
      ...
      if (_pdfjsLib.PDFJS.disableAutoFetch && percent) {
        ...
        this.disableAutoFetchLoadingBarTimeout = setTimeout(function () {
          ...
          this.eventBus.dispatch('hideLoadingBar');
  ...
  load: function load(pdfDocument, scale) {
    ...
    pdfDocument.getDownloadInfo().then(function () {
      ...
      _this6.eventBus.dispatch('hideLoadingBar');
      firstPagePromise.then(function () {
        _this6.eventBus.dispatch('documentload', { source: _this6 });
      });
    });
    var pageModePromise = pdfDocument.getPageMode().catch(function () {});
    if (!this.toolbar.setExternal) {
...
var validateFileURL = void 0;
{
  ...
  var HOSTED_VIEWER_ORIGINS = ['https://s3.amazonaws.com', 'http://mozilla.github.io', 'https://mozilla.github.io'];
...
// This is what is triggered when a new number is entered in the pager and enter is pressed.
...
function webViewerLoad() {
  var event = new CustomEvent("pdfjsLoadedViewer");
